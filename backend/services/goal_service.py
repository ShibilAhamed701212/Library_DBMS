...
def get_or_create_goal(user_id):
    ...
    if not goal:
        execute("INSERT INTO reading_goals (user_id, year, target_count) VALUES (%s, %s, 12)", (user_id, year))
        goal = fetch_one(query, (user_id, year))
...
def update_goal_target(user_id, target):
    ...
    year = datetime.now().year
    execute("UPDATE reading_goals SET target_count = %s WHERE user_id = %s AND year = %s", (target, user_id, year))
...
def increment_goal_progress(user_id):
    ...
    execute("UPDATE reading_goals SET current_count = current_count + 1 WHERE user_id = %s AND year = %s", (user_id, year))
